...
# Load img sound
img_sound = pygame.image.load('sound.png').convert_alpha()
img_unsound = pygame.image.load('unsound.png').convert_alpha()
...
class Player(pygame.sprite.Sprite):
        def __init__(self):
            ...
            self.playerImages = [pygame.image.load("JiJiSR1.png").convert_alpha(),
                                 pygame.image.load(
                "JiJiSR1L.png").convert_alpha(),
                pygame.image.load(
                "JiJiSR1R.png").convert_alpha()
            ]
        ...
        def shoot(self, allsprites, cures):
            cure_sound.play()
...
class Buff(pygame.sprite.Sprite):
        def __init__(self, buff_type):
            ...
            self.image = pygame.image.load(
                f"buff{buff_type}.png").convert_alpha()
        ...
        def apply_buff(self, player, covids, allsprites):
            if self.buff_type == 1:
                ...
                spawn_covid(5, allsprites, covids)
                # clear buff
                self.kill()
...
class Boss(pygame.sprite.Sprite):
    def __init__(self, all_sprites_group, boss_lasers_group):
        super().__init__()
        ...
        self.allsprites_group = all_sprites_group
        self.boss_lasers_group = boss_lasers_group
    ...
    def update(self):
        ...
        if now - self.last_shot > self.shoot_delay:
            ...
            self.allsprites_group.add(laser)
            self.boss_lasers_group.add(laser)
...
def main_menu():
    ...
    img_bg = pygame.image.load('BG_main.png').convert_alpha()
...
def level_set():
    img_bg_in_game = pygame.image.load('BG_in_game.png').convert_alpha()
    resize_bg_in_game = pygame.transform.scale(img_bg_in_game, (WIDTH, HEIGHT))
    screen.blit(resize_bg_in_game, (0, 0))


def spawn_covid(amount, allsprites, covids):
...
def player_loss(player):
    ...
    loss_img = pygame.image.load('lose.png').convert_alpha()
...
async def main_1():
    global GAME, main, total_levels, level1, boss1, doctor, img_value, timer, amount_covids, pause, timecount
    while GAME:
        ...
        while total_levels:
            ...
            total_levels_img = pygame.image.load(
                'intro.png').convert_alpha()
        ...
        if level1:
            ...
            spawn_covid(amount_covids, allsprites, covids)
            ...
            while level1:
                ...
                if not pause:
                    ...
                    for buff in buff_hits:
                        item.play()
                        buff.apply_buff(player, covids, allsprites)
                    ...
                    # Output
                    bg_in_game = pygame.image.load(
                        'BG_in_game.png').convert_alpha()
                    ...
                    if player.life <= 0:
                        ...
                        img_buff4 = pygame.image.load(
                            "buff4.png").convert_alpha()
        ...
        if boss1:
            ...
            boss = Boss(allsprites, boss_lasers)

            allsprites.add(player)
            allsprites.add(boss)
            print(doctor)
            # make allsprites to grobal variable
